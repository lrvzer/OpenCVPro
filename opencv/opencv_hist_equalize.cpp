...
int main(int argc, char const* argv[])
{
    ...
    cv::equalizeHist(gray, dst); // 只能操作灰色图像
    cv::imshow("dst", dst);

    cv::Mat b_hist, g_hist;
    int hintSize = 256;
    float range[] = { 0, 255 };
    const float* hintRanges = { range };
    cv::calcHist(&gray, 1, 0, cv::Mat(), b_hist, 1, &hintSize, &hintRanges, true, false);
    cv::calcHist(&dst, 1, 0, cv::Mat(), g_hist, 1, &hintSize, &hintRanges, true, false);
